...
| **Whole page reloads on every update** (`Received docShutdown` → `reloadPane` in console) | Server-side, not the widget: with access rules on the target table, Grist can't stream the change incrementally and orders all viewers to reload. | Confirm by editing `Last_Seen` by hand in the grid — if that reloads too, it's the doc rules. Mitigations: keep the update threshold coarse (default: once per day per viewer, see `STALE_THRESHOLD_MS`), and avoid rule conditions on the timestamp column. |
