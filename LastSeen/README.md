...
| **Whole page reloads on every update** (`Received docShutdown` → `reloadPane` in console) | Server-side, by design: this doc sources access-rule **user attributes** from the same table the widget writes (e.g. `Team_member` matched by Email). Grist snapshots each viewer's attribute row and, on any write touching it, throws `NEED_RELOAD` ("user attributes changed") — your own `Last_seen` cell is part of that row — then orders every client to reload. No widget code, write path, or threshold avoids it while that config stands. | Fix in the doc, pick one: (1) **Preferred:** in Access Rules, point user attributes at a *static* table the widget never writes (e.g. a team directory), so `Last_seen` updates stream incrementally with no reload. Verify by hand-editing `Last_seen` in the grid — no reload means fixed. (2) Track presence in a separate rule-free log table instead of updating `Last_seen` in place. |
